...
CREATE OR ALTER PROCEDURE DW.InsertUpdateDelete_SpotifyTracks
AS
BEGIN
    SET NOCOUNT ON;
    ...
    BEGIN TRY
        ...
        WHERE NOT EXISTS (
            SELECT 1
            FROM Landing.SpotifyTracks l
            WHERE l.track_id = d.TrackID
        );

        INSERT INTO Config.LoadAudit
        (ProcedureName, LoadType, Status)
        VALUES
        ('DW.InsertUpdateDelete_SpotifyTracks', 'Insert Update Delete', 'Success');
    ...
    BEGIN CATCH
        ...
        VALUES
        ('DW.InsertUpdateDelete_SpotifyTracks', 'Insert Update Delete', 'Failed', ERROR_MESSAGE());
    END CATCH
END;
GO